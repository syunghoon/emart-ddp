...
export default function PhotoPage() {
  ...
  return (
    <main className="relative flex flex-col items-center h-screen px-[3vh] pt-[3vh] pb-[3vh]">
      {!captured && (
        <>
          <div className="relative w-full aspect-[3/4] overflow-hidden mt-[10vh]">
            {/* 웹캠 프리뷰 */}
            <Webcam
              ref={webcamRef}
              audio={false}
              ...
              videoConstraints={{ width: 375, height: 500, facingMode: "user" }}
          ...
          {/* 하단 카운트다운 버튼형 표시 */}
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-[#2a0f5a] text-white rounded-full w-20 h-20 flex items-center justify-center text-2xl font-semibold shadow-lg">
            {preCountdown}
          </div>
        </>
      )}
      ...
      {captured && (
        <>
          ...
          <button
            ...
            aria-label="홈으로"
            className="absolute top-[2vh] right-[2vh] z-50 w-[5vh] h-[5vh] rounded-full bg-white/15 border border-white/30 text-white text-[1.6rem] leading-none flex items-center justify-center hover:bg-white/20"
          >
            ×
          </button>
          {/* 캡처 프리뷰 (프레임 포함) */}
          <div className="relative w-full aspect-[3/4] mt-[10vh]">
            <img
              src={captured}
              alt="Captured"
              className="absolute inset-0 w-full h-full object-cover"
            ...
            <img
              ...
              className="absolute inset-0 w-full h-full pointer-events-none"
            />
            {/* 프레임 위 중앙 하단: 다시 찍기 버튼 */}
            <button
              onClick={retake}
              className="absolute left-1/2 -translate-x-1/2 bottom-[-5%] px-[3.2vh] py-[1.6vh] rounded-full text-white border-2 border-white/85 bg-white/10 backdrop-blur-sm shadow-[0_0.8vh_2vh_rgba(0,0,0,0.35)] flex items-center gap-[1vh]"
            >
              <span className="text-[1.4rem]">↻</span>
              <span className="text-[1.2rem] font-semibold">다시 찍기</span>
            </button>
          </div>
          ...
          <div className="mt-[5vh] w-full">
            <button
              onClick={printImage}
              className="w-full py-[2.2vh] rounded-full bg-white text-[#160449] font-ohsquare text-[1.35rem] font-extrabold shadow-[0_0.8vh_2vh_rgba(0,0,0,0.35)]"
            >
              사진 출력하기
            </button>
          </div>
